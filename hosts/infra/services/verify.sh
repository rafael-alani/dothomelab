...
read -r paperless_route paperless_gpt_route prometheus_route loki_route \
  ...
  n8n_route pulse_route shelfarr_route listenarr_route cleanuparr_route sortarr_route bookorbit_route grimmory_route storyteller_route \
...
expected = {
    ...
    '["listenarr.rafael.media"]': ("192.168.0.102", 4545),
    '["cleanuparr.rafael.media"]': ("192.168.0.102", 11011),
    '["sortarr.rafael.media"]': ("192.168.0.102", 9595),
...
[[ "$paperless_route" == "1" && "$paperless_gpt_route" == "1" &&
  ...
  "$shelfarr_route" == "1" && "$listenarr_route" == "1" &&
  "$cleanuparr_route" == "1" && "$sortarr_route" == "1" &&
  "$bookorbit_route" == "1" &&
...
read -r homarr_integrity homarr_apps homarr_items homarr_layouts \
  ...
  homarr_syncthing_items homarr_cleanuparr_app homarr_sortarr_app \
  homarr_sortarr_items homarr_listenarr_app homarr_listenarr_items < <(
...
with sqlite3.connect(f"file:{sys.argv[1]}?mode=ro", uri=True) as connection:
    ...
    expected_layouts = 26 * connection.execute(
        """
        SELECT count(*)
        FROM layout
    ...
    syncthing_items = connection.execute(
        ...
        WHERE item.id IN (
          ...
          AND board.name IN ('dashboard', 'Admin', 'HomeAssistant')
    ...
    sortarr_app = connection.execute(
        """
        SELECT count(*)
        FROM app
        WHERE id = 'dhlsortarrapp000000000001'
          AND name = 'Sortarr'
          AND href = 'https://sortarr.rafael.media'
          AND ping_url = 'http://192.168.0.102:9595'
        """
    ).fetchone()[0]
    sortarr_items = connection.execute(
        """
        SELECT count(*)
        FROM item
        JOIN board ON board.id = item.board_id
        WHERE item.id IN (
          ...
          AND board.name IN ('dashboard', 'Admin', 'HomeAssistant')
          AND item.options LIKE
            '%"appId":"dhlsortarrapp000000000001"%'
          AND item.options LIKE '%"pingEnabled":false%'
        """
    ).fetchone()[0]
    listenarr_app = connection.execute(
        """
        SELECT count(*)
        FROM app
        WHERE id = 'dhllistenarrapp000000001'
          AND name = 'Listenarr'
          AND href = 'https://listenarr.rafael.media'
          AND ping_url =
            'http://192.168.0.102:4545/api/v1/configuration/bootstrap'
        """
    ).fetchone()[0]
    listenarr_items = connection.execute(
        """
        SELECT count(*)
        FROM item
        JOIN board ON board.id = item.board_id
        WHERE item.id IN (
          ...
          AND board.name IN ('dashboard', 'Admin', 'HomeAssistant')
          AND item.options LIKE
            '%"appId":"dhllistenarrapp000000001"%'
          AND item.options LIKE '%"pingEnabled":true%'
        """
    ).fetchone()[0]
print(
    integrity,
...
printf 'OK Homarr Sortarr link is managed and unauthenticated tile pings are disabled\n'
printf 'OK Homarr Listenarr link and direct bootstrap health ping are managed\n'
