...
Application guests are `servarr` (102), `infra` (110), and `apps` (112), plus VM 101 and HAOS VM 104. The protected PBS appliance remains in LXC 113. Pi-hole keeps service address `192.168.0.100` on `infra`; Jellyfin and Seerr run as the independent `media` project on `apps`.
