...
| Rollback needs multiple independent layers | Old images/volumes, a ZFS snapshot, persistent counts, and PBS were retained. | Root volumes were copied while stopped, byte-compared, snapshotted, and backed up before replacement. | The active Apps state is in a verified post-cleanup PBS snapshot, but Immich dumps/snapshots/containers and selected rollback assets remain until a focused Apps restore passes. |
