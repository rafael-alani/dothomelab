...
9. Generates a fresh internal Docker API CA, configures mutual TLS, deploys all
   twenty-eight Compose projects, reconciles the private Paperless, Prometheus,
   ...
   Pulse, Syncthing, and Storyteller routes, configures their exact Pi-hole records,
...
10. Recaptures native credentials/state and runs `provision/verify.sh`, including
   storage, all 63 containers, service APIs, database/application counts,
