...
8. Generates a fresh internal Docker API CA, configures mutual TLS, deploys all
   sixteen Compose projects, reconciles the private Paperless, Prometheus,
   Loki, ImmichFrame, Wizarr, Bar Assistant, and yt-dlp NPM routes and Homarr
...
9. Recaptures native credentials/state and runs `provision/verify.sh`, including
   storage, all 47 containers, service APIs, database/application counts,
